...
for file in "${HTML_FILES[@]}"; do
  ...
  # Use sed to fix the file - this pattern looks for the main tag inside the header tag
  # and properly closes the header tag before the main tag
  sed -i '' 's/\(<div class="header-actions">.*<\/div>\s*<\/div>\s*<\/div>\)\s*\(<main id="main-content">\)/\1\n<\/header>\n\n\2/g' "$file"
